...
**해당 없음 — UI 변경이 없다.** 렌더링되는 컴포넌트, props, 상태 어느 것도 건드리지 않는다.
...
조인 결과는 테이블 타입으로 떨어지지 않으므로 **쿼리에서 역추출**한다.

```text
QueryData<typeof postWithSeriesQuery>    ← @supabase/supabase-js
```

이를 위해 조인 쿼리는 인라인이 아니라 **한 곳에 선언해 타입과 실행에 함께 쓴다.** 현재 `getPostBySlug`와 `getPublishedPostBySlug`가 동일한 `select("*, series(title, slug, categories(name, slug))")`를 중복 보유하는데, 여기서 자연스럽게 하나로 모인다.
