...
### ~~dead code 3건~~ ✅ 완료 (2026-07-22)

typegen-1이 남긴 잔재 정리(deadcode-1). 스코프 A(명시 dead code만).

- ✅ `src/shared/types/index.ts` — `Post`/`PostFrontmatter` 중복 정의(소비처 0, entities 버전보다 낡음) → 파일 + 빈 디렉토리 삭제
- ✅ `entities/post/index.ts:3`의 `toPost` re-export(배럴 밖 소비처 0) → 라인 삭제. 실사용은 `api/posts.ts`의 `../model/mappers` 직접 import라 불변
- ✅ `.claude/worktrees/fix+ci+pnpm-version-ssot` — 조사 시 **이미 비어 있음**(`git worktree list` main만) → 실효, 별도 작업 없음

설계서 `deadcode-1_typegen-residue-cleanup.md`. → 상세는 [변경 이력](#변경-이력) 참조.
...
- 2026-07-22: **deadcode-1 완료 — typegen-1 잔재 dead code 정리** — 순수 삭제 리팩터(스코프 A, 동작 불변). ① `src/shared/types/index.ts` 삭제(+ 빈 `src/shared/types/` 디렉토리 제거) — `Post`/`PostFrontmatter` 중복 정의였으나 전체 `src` 소비처 0, 정본은 DB 생성 타입 파생 `entities/post/model/types.ts`(nullable 정직·컬럼 정합). ② `entities/post/index.ts`의 `export { toPost } from "./model/mappers"` 라인 삭제 — 배럴 경유 외부 소비처 0(실사용 `api/posts.ts`는 `../model/mappers` 직접 import). 배럴의 나머지 4개 export(`PostCard`/`PostMeta`/`Post`/`PostFrontmatter`/`PostWithSeries`)는 유지. `PostFrontmatter`는 미소비이나 엔티티의 정당한 공개 타입이라 유지(스코프 B 제외). ③ `.claude/worktrees` — 조사 시 이미 비어 있어(`git worktree list` main만) 실효, 작업 없음. 삭제 직전 grep 소비처 0 재확인 + `tsc --noEmit`/`pnpm lint` 신규 위반 0으로 소비처 0 실증. 설계서 `deadcode-1_typegen-residue-cleanup.md`. **남은 후속: migration-1(우선순위 낮음).**
