...
| 2026-06-29 | **C-1**: autofix 8룰 신규 error 도입 | `react/jsx-no-useless-fragment`·`react/self-closing-comp`·`import/{order,first,newline-after-import}`·`prefer-template`·`unicorn/{throw-new-error,no-await-expression-member}`. `import/order`는 alphabetize ON + pathGroups(react/next 앞, `@/` internal, newlines-between always). `pnpm lint:fix` 일괄 정리 ~43 파일(import 재정렬 + prefer-template 3, 로직 0). 위반은 import/order 89 + prefer-template 3뿐 전부 autofix, 잔여 0. baseline 13 유지(승격 룰 0 warning) |
| 2026-06-29 | **B-1**: Tier B 3룰 + 제어흐름 2룰 error 승격 | `sonarjs/cognitive-complexity` warn15→**error 10**(기본값 15보다 엄격, 최복잡 함수=9 통과), `unicorn/prefer-string-replace-all`·`react/jsx-no-leaked-render`(ternary) warn→error, `no-else-return`·`no-nested-ternary` **신규 error**(제어흐름 표준). 중첩삼항 3곳 해소: `HomePage.tsx`·`TechDetailPage.tsx` `tapeColor` → 로컬 `TAPE_COLOR` Record, `PostForm.tsx` 버튼 라벨 → 모듈 레벨 `submitLabel()` early-return. 프로젝트 `CLAUDE.md` "Control Flow & Conditionals" 절 신설. baseline 13 유지(승격 룰 전부 0 warning) |
| 2026-06-30 | **C-2**: type-aware 룰 2종 신규 error 도입 | **전제 인프라**: `eslint.config.mjs` 공통 블록에 `languageOptions.parserOptions.projectService: true` + `tsconfigRootDir`(typescript-eslint v8 권장, project 배열보다 빠름) — 향후 Tier S 전제도 확보. `@typescript-eslint/prefer-nullish-coalescing`·`prefer-optional-chain` **신규 error**. optional-chain 위반 0(코드 수정 없음). nullish 위반 5건은 전부 빈 문자열("") 폴백 의도 → `??` 치환 시 동작 변경(빈 redirect 이동·빈 토스트·빈 라벨·빈 FK 저장)이라 **line-level `eslint-disable-next-line` + 사유 주석**으로 의도 명시(동작 보존, 룰 미해제): `route.ts(×2)` series_id·`login/page.tsx` redirect·`PostForm.tsx` data.error·`StripePlaceholder.tsx` label. 회귀 검증: nullable `\|\|`/중첩 `&&` 추가 시 error 확인. baseline 13 유지(error라 warn 미증가) |
| 2026-06-30 | **S-1**: async 안전성 type-aware 4룰 신규 error 도입 | `@typescript-eslint/no-floating-promises`·`no-misused-promises`·`await-thenable`·`require-await` **신규 error**(C-2의 `projectService` 인프라 활용). no-misused-promises 4건은 async 핸들러를 JSX void 위치에 전달한 케이스 → **인라인 `void` 래핑**으로 해소(`(e) => void fn(e)`, 핸들러 내부 try/catch 완비라 동작 보존): `LoginForm.tsx:46` onSubmit·`MdxEditor.tsx:93` onPaste·`PostForm.tsx:146` onSubmit·`PostForm.tsx:283` onClick. 나머지 3룰 위반 0. 회귀 검증: 처리 안 한 Promise→`no-floating-promises`, await 없는 async→`require-await` error 확인. `no-unnecessary-condition`(17건)은 **Tier D로 분리**. baseline 13 유지 |
| 2026-07-16 | **tierD-1**: `@typescript-eslint/no-unnecessary-condition` 신규 error 도입 (Tier D 재판정) | Tier D가 기각(untyped Supabase 방어 가드 오판)했으나 **typegen-1로 타입이 정직해져 재판정** → error 채택. 위반 12건 중 **11건은 진짜 dead**가 되어 단순화: `entities/{post,series,category}/api` `if (error \|\| !data)` → `if (error)` 10건(discriminated union상 error 검사 후 data non-null 좁힘, tsc 검증), `series/model/mappers.ts` `Number(post_count ?? 0)` → `Number(post_count)` 1건(RPC 반환 타입 non-null + count() null 미반환). **1건은 `navigator.clipboard?.`**(DOM 타입이 non-null로 거짓, 비-HTTPS/구형에서 undefined) → **line-level `eslint-disable` + 사유**로 예외(단일행 형태만 유효, 2줄 `//`는 unused-directive). `prefer-nullish` disable 5건은 빈 문자열 폴백 의도로 재확인(변경 0). 프로젝트 `CLAUDE.md`에 "Type-aware rule exceptions" 메타 컨벤션 신설. 회귀 검증: dead 조건 복원 시 error 확인. 매퍼 특성화 테스트 15/15 GREEN(동작 보존). baseline 13 유지 |
