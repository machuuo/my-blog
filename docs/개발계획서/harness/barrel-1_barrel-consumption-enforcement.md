...
- 이 과정에서 `shared/ui/index.ts` 신설, shared 딥 import 69건 교체, dead 세그먼트 배럴 9개 제거를 함께 수행한다.
...
- **dead 배럴 9개 = 이 PR에 포함**: 소비 0건 확인 + FSD 비표준. 배럴 정비하는 김에 함께 제거. diff가 강제규칙+정리로 섞이는 단점은 수용(규모 작음).
...
- **server-only 예외 = 유지**: `auth`/`supabase/*`/`entities/*/server`는 배럴 제외가 의도(server/client 번들 경계). entry-point allow로 명시.
